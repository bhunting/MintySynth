...
void setup()
{
  ...
  {
    MIDIstate = 0;
  } // if it's greater than 2 we've never saved it before, so set it to 0.
  if (pressed[1])
  {
    toggleMIDI();
  } // toggle it if button 1 is held.
  if (MIDIstate != 0)
  {
    MIDI.begin();
  } // this defaults to the standard MIDI baud rate of 31250.
  if (MIDIstate == 2)
  {
    Serial.begin(115200);
  } // use MIDI state 2 for sending MIDI via serial to PC (e.g. using Hairless MIDI)
...
void loop()
{
  ...
  { // we only need to read the pots every once in a while.
    get_pots();
    readTime = millis();
  ...
  if (version == 2)
  {
    if (LDRMax > 500)
    {
      ...
      {
        LDRread = constrain((map(analogRead(5), LDRMax - 500, LDRMax, 1023, 0)), 0, 1023);
      }
    } // if we're in LDRMode we read the LDR any time we can as long as the LEDs aren't turned on.
  ...
  if (((millis() - triggerTime) >= 70) && thisStep == 1 && LED2state == 1 && !(sequMode && livePots[4]) && !envMode && !pause)
  {                     // turn off the LEDs after some time if they're on and we're not playing sample scales. If we're on step 1 we leave the LED on longer.
  ...
  if (version == 2)
  {
    if (pressed[1] && pressed[2] && programMode == 4)
    { // enter LDR Mode if buttons 0 and 1 are pressed.
      modeChange = 2;
      LDRMode = 1;
      livePots[1] = 0;
      LDRMax = analogRead(5);
  ...
  if (version == 2)
  {
    ...
    { // enter tripwire Mode if buttons 2 and 3 are pressed and we're in Live Mode, or if we're already in Tripwire Mode and we just want to recalibrate.
      ...
      PORTD &= ~_BV(PD4);
      for (int i = 0; i < 3; i++)
      { // flash LED 1 to indicate beginning of calibration period
        delay(100);
        PORTD |= _BV(PD2);
        delay(25);
        PORTD &= ~_BV(PD2);
      ...
      while (((millis() - LDRTimer) < 5000) && tripwireMode)
      { // calibrate the LDR during the first five seconds of tripwire Mode. Taken from: https://www.arduino.cc/en/Tutorial/Calibration
      ...
      for (int i = 0; i < 3; i++)
      { // flash LED 2 to indicate end of calibration
        delay(100);
        PORTD |= _BV(PD4);
        delay(25);
        PORTD &= ~_BV(PD4);
  ...
  if (potLockFlag && livePots[0] && !instrumentProgMode)
  { // if we've locked POT 0 because one of the buttons is held, and POT 0 becomes live again, we've turned it and we are in Wave Program Mode.
    wavProgMode = 1;
  ...
  if (wavProgMode && ((millis() - waveTimer) >= 1500))
  {            // if we've been in wavProgram mode for a bit,
  ...
  {
    potLockFlag = 0;
  } // if there's no modeChange flag, the button has been released, so unflag the pot.

  for (int i = 0; i < 4; i++)
  {
    if (longpress[i] && programMode == 4 && !wavProgMode && !potLockFlag && !instrumentProgMode && modeChange == 0)
    { // if a button is held, lock POT0 and wait to see if it's turned, telling us to enter Wave Program Mode.
      livePots[0] = 0;
      potLockFlag = 1;
  ...
  if (wavProgMode && modeChange == 0)
  { // if there's no modeChange flag, it's because a button has been released, and it's time to exit wavProgMode.
    wavProgMode = 0;
  ...
  if (potLockFlag1 && livePots[1] && !wavProgMode)
  { // if we've locked POT 1 because one of the buttons is held, and POT 1 becomes live again, we've turned it and we are in Instrument Program Mode.
  ...
  for (int i = 0; i < 4; i++)
  {
    if (longpress[i] && programMode == 4 && !instrumentProgMode && !wavProgMode && !potLockFlag1 && modeChange == 0)
    { // if a button is held, lock POT1 and wait to see if it's turned, telling us to enter Instrument Program Mode.
      livePots[1] = 0;
      potLockFlag1 = 1;
    } // the flag tells us we've locked the pot and we're waiting...
  ...
  if (instrumentProgMode && modeChange == 0)
  {                  // if there's no modeChange flag, it's because a button has been released, and it's time to exit instrumentProgMode.
  ...
  {
    if (justreleased[i])
    {                           // if a button was just released and we're in not in Mixer Mode or Sequencer Mode,
      userWave = switchWave[i]; // set the waveform
      if (MIDIswitchChannel[i] != currentChannel)
      {
        MIDI.sendControlChange(123, 0, currentChannel);
      } // if we're switching channels, we turn off all notes on the previous channel, so none accidentally get stuck on.
  ...
  if (((millis() - envTimer) >= 1500) && envMode)
  {                     // if we haven't changed the envelope for some time...
  ...
  if (longpress[4] && !mixerMode)
  {
    ...
    {
      modeChange = 1;
    } // set a modechange flag unless there's already one set
    lock_pots();
  }
  ...
  if (mixerMode && modeChange == 0)
  { // if there's no modeChange flag, it's because a button has been released, and it's time to exit mixerMode.
    mixerMode = 0;
    lock_pots();
  ...
  for (int i = 0; i < 4; i++)
  {
    if (longpress[i] && !sequMode && programMode < 4)
    { // enter Sequencer Mode when we hold down button 0-3
      sequMode = 1;
      voice0Volume = volume[0]; // remember the current volume of voice 0
      if (modeChange == 0)
      {
  ...
  if (sequMode && livePots[4])
  {
    ...
    {
      scaleChange = 1;
    } // set a flag if the scale has changed.
    else
    {
      scaleChange = 0;
    }
    ...
    if (scaleChange)
    { // reset the scale to the beginning if the scale has changed.
      scaleNote = 0;
      ascending = 1;
    }

    if ((((millis() - triggerTime) >= 100) && (scaleNote > 0 || !ascending)) || (scaleNote == 0 && ((millis() - triggerTime) >= 500) && ascending))
    { // trigger the next sample scale note if enough time has passed.
      ...
      {
        ascending = 0;
      } // reverse the scale when we get to the top.
      if (ascending)
      {
        scaleNote++;
      } // increment the scale note if we're going up.
      if (!ascending && scaleNote == 0)
      {
        ascending = 1;
      } // reverse the scale again when we get to the bottom.
      if (!ascending && scaleNote > 0)
      {
        scaleNote--;
      } // decrement the scale note if we're going down.

      triggerTime = millis();
    }
  }

  if (sequMode && modeChange == 0)
  { // if there's no modeChange flag, it's because a button has been released, and it's time to exit sequMode. We also make sure we're set up to start the next sample scale at the beginning.
    sequMode = 0;
    pause = 0;
    volume[0] = voice0Volume; // turn voice 0 back down if necessary.
  ...
  for (int i = 0; i < 4; i++)
  { // enter Program Mode when we hold down button 4 and press button 0-3
  ...
  { // when in Program Mode we use the buttons to set the waveform and write it to the voicePrefs array.
    for (int i = 0; i < 5; i++)
    {
      if (justreleased[i])
      {
  ...
  {
    voicePrefs[2][programMode][4] = userMod;
  } // set modulation if we're in Mixer Mode

  if (pressed[0] && justpressed[4] && programMode < 4)
  { // exit Program Mode (enter Live Mode) if we hold down 0 and press 4.
    modeChange = 2;
    programMode = 4;
  ...
  if (modeChange == 0 && programMode == 4 && !wavProgMode && !instrumentProgMode && !pressed[4] && extralongpress[0] && !pressed[1] && !pressed[2] && !pressed[3])
  {                 // button 0
    modeChange = 1; // only one button pressed to trigger load(), so we set one modechange flag.
    load(0);
  }
  ...
  if (((millis() - shiftTimer) >= 1500) && shiftMode)
  { // exit shift mode
    shiftMode = 0;
    PORTD &= ~_BV(PD2); // turn off the LEDs
    PORTD &= ~_BV(PD4);
  ...
  if (((millis() - batteryTime) >= 60000) && batteryLow == 0 && !LED1state && !LED2state)
  { // we check the battery at startup and then once/minute unless the battery has already been flagged as low.
    check_battery();
    batteryTime = millis();
  }
  ...
  if ((millis() - triggerTime) >= swingLength && !pause)
  { // check to see if the previous step is done and the music is not paused. If so, reset the step timer, set up the voices, and trigger the next step.
    ...
    if (!mixerMode && !sequMode && !wavProgMode && !instrumentProgMode)
    {
      ...
      {
        scalePitch = constrain((map((LDRread), 0, 1023, 20, 106)), 20, 105);
      } // if we're in LDRMode we read pitch from the LDR instead of POT1.
    ...
    if (scaleNumber > 0)
    {
      userPitch = (pgm_read_byte(&(scale[scaleNumber - 1][scalePitch]))) + scaleShift;
    } // here we map to the scale lookup tables if a scale other than chromatic is selected.
    if (scaleNumber == 0)
    {
    ...
    if ((thisStep % 2) == 0)
    {
      swingLength = stepLength + ((stepLength * Swing) >> 7);
    } // lengthen even steps by the swing percentage and shorten odd steps by the swing percentage.
    else
      swingLength = stepLength - ((stepLength * Swing) >> 7);

    if (swingLength > (stepLength * 2))
    {
      swingLength = stepLength;
    } // just to make sure that we never lengthen the even steps by more than 100%

    // Set up the voices:

    for (int i = 0; i < 3; i++)
    { // set everything except pitch from the prefs array for each voices 0-2.
    ...
    if (programMode < 4)
    { // if we're in Program Mode 0-3 we want to read the voice 3 prefs from the array, just like the other voices.
    ...
    { // if we're in Live Mode we want to read the voice 3 prefs directly from the pots, not from the stored arrays.
    ...
    {
      edgar.mTrigger(0, song[Current][0][thisStep]);
    } // trigger voices 0-2, getting the pitch from the song array. If the value in the array is 0 or if the duration is very short, no note is played.
    ...
    {
      edgar.mTrigger(3, song[Current][3][thisStep]);
    } // if we're in Program Mode 0-3 we want to read the voice 3 pitch from the array, just like the other voices.
    else if (song[Current][3][thisStep] != 0 && volume[3] != 13 && userLength > 3)
    {
      edgar.mTrigger(3, userPitch);
    } // if we're in Live Mode we'll read voice 3 pitch from the Pot or LDR.
    ...
    {
      thisStep = 0;
    } // reset the counter after 16 steps

    if ((thisStep == 0) && (Current == 0))
    {
      Current = 1;
    } // if we're playing the demo song, which is 32 steps long, toggle between the two 16-step halves.
    ...
    {
      Current = Current + 1;
    } // if we've appended multiple songs, increment the currently playing song.

    if (thisStep == 0 && appendMode == 2 && Current == 4)
    {
      Current = 2;
    } // if we've appended two songs, return to the first after the second has played.
    if (thisStep == 0 && appendMode == 3 && Current == 5)
    {
      Current = 2;
    } // if we've appended three songs, return to the first after the third has played.
