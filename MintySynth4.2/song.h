...
uint8_t voicePrefs[6][4][7] = // the voice parameters for each of the six songs and four channels (voices, instruments). 
                              // The 5 parameters are wave[0-11], pitch[0-127], envelope[0-3], length[0-127], mod[0-127:64=no mod], 
                              // MIDI channel, MIDI instrument[0-127]
    // if you turn on the printSongs() call in setup you can print your own song to the serial moitor and then paste it in below 
    // to change the demo song to your own.
{
    ...
    {{10, 60, 1, 60, 64, 1, 0}, // voices for song2 - song 2 is the one that we write to when we're sequencing. 
    {10, 64, 1, 60, 64, 1, 0}, // It's initially set up to play the reference notes for each voice when we start the device in Program Mode.
    {10, 67, 1, 60, 64, 1, 0},
    {10, 72, 1, 60, 64, 1, 0}},
...
uint8_t song[6][4][16] = // dimension 1 is the song, dimension 2 is the voice (channel), dimension 3 is the MIDI note. 
                         // A zero value is a rest (no note played).
{
