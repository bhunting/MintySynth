...
void check_pots()
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    if (livePots[i] == 0 && firstRead[i] == 0)
    {
      ...
      firstRead[i] = 1;
    } // note that we've taken a first reading

    if (((POT[i] < 2000 && previousReading[i] < 2000) && (POT[i] > ((previousReading[i] + 10)) || ((POT[i] < (previousReading[i] - 10))))))
    { // Get another value to see if the pot has been turned more than ~1/10th revolution. If so, unlock the pot.
      livePots[i] = 1;
      firstRead[i] = 0;
    } // now that the pot is unlocked we'll need a new initial reading next time it's locked.
  }
}
...
void get_pots()
{
  for (int i = 0; i < 5; i++)
  {
    ...
    if (POT[i] > 1023)
    { // if for some reason we get a wacky reading,
      POT[i] = analogRead(i);
...
void save(int bank)
{
  ...
  PORTD &= ~_BV(PD4);
  for (int i = 0; i < 4; i++)
  { // flash LED 1 to indicate a save
    PORTD |= _BV(PD2);
    delay(25);
    PORTD &= ~_BV(PD2);
    delay(200);
...
void load(int bank)
{
  ...
  PORTD &= ~_BV(PD4);
  for (int i = 0; i < 4; i++)
  { // flash LED 2 to indicate a load
    PORTD |= _BV(PD4);
    delay(10);
    PORTD &= ~_BV(PD4);
    delay(100);
